...
export default function DeletePage() {
  ...
  const loadGallery = () => {
    axios.get("http://localhost:3001/gallery").then((res) =>
      setGallery(
        res.data.sort((a, b) => b.name.localeCompare(a.name)) // newest first
      )
    );
  };
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 w-full">
              {gallery.map((item, idx) => (
                <div
                  ...
                  className="relative border border-white rounded-md overflow-hidden cursor-pointer hover:scale-105 transition"
                >
